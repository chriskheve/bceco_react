...
import { getTotalPaiement } from "../../../../api/paiement";

export function StatsWidget12({ className }) {

  const [total_facture_paiement, setTotalPaiementsFacture] = useState([])
  ...
  const fetchData = React.useCallback(() => {
    ...
    getTotalPaiement()
    .then(response => {
      setTotalPaiementsFacture(response.data.totalPaiementsFacture)
        ...
        <div className="d-flex align-items-center justify-content-between card-spacer flex-grow-1">
          ...
          <div className="d-flex flex-column text-right">
            <span className="text-dark-75 font-weight-bolder font-size-h3">
              {total_facture_paiement} FC
            </span>
            <span className="text-muted font-weight-bold mt-2">Revenu des factures</span>
          </div>
        </div>
        <div
